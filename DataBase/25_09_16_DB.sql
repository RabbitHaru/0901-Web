...
-- where 절에서 사용하는 연산자 ( 비교 연산자 = , != , <> )
-- emp 테이블에서 sal 이 3000 인 데이터를 조회
select *
from emp
where sal = 3000;
...
-- emp 테이블에서 sal(급여)가 2000 이상 3000 이하인 데이터를 조회
-- 1. between 을 사용 안하고 작성
select * from emp
...
where ename like '%AM%';
-- like 의 부정( not like )
-- emp 테이블에서 AM 을 포함하지 않는 사원의 이름을 출력하여라
select ename
from emp
where ename not like '%AM%';

-- IS NULL / IS NOT NULL 연산자
-- NULL 이란? 저장된 값(데이터)이 없을을 뜻한다.
...
-- 모든 결과에 대해서 1번 조건이 false지만, 2번 조건이 true가 되는게 있기 때문에 출력값이 있다.

-- 퀴리 조회문을 정렬하기 ( ORDER BY 절 )
...
-- empno -> employee.no / ename -> employee.name / mgr -> manger / sal -> salary
-- 부서 번호를 기준으로 내림차순 정렬하되, 부서 번호가 같으면 사원 이름을 기준으로 오름차순 하라.
select empno as "employee.no", ename as "employee.name", job, mgr as "manger",
hiredate, sal as "salary", comm, deptno
from emp
order by deptno desc, ename asc;

-- 3. EMP 테이블에서 사원이름이 S로 끝나는 사원 데이터를 모두 출력하여라.
select *
from emp
where ename like '%S';
...
-- 6. EMP 테이블에서 급여 값이 2000이상 3000 이하 범우 외의 값을 가진 사원의 이름, 급여를 조회하라.
select ename, sal
from emp
