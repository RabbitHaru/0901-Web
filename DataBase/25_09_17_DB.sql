...
-- 문자 함수
-- 1. INITCAP() 함수 : 첫 글자만 대문자로 출력하고, 나머지는 전무 소문자로 출력
...
FROM DUAL;
-- EMP 테이블에서 SCOTT 이라는 데이터를 찾으려고 할 때 사용 가능
-- 사용자가 소문자로 적을지 대문자로 적을지 알 수 없기 때문에 UPPER 또는 LOWER로 일치 시켜준다.
SELECT *
FROM EMP
WHERE UPPER(ENAME) LIKE UPPER('scott');
-- LENGTH(), LENGTHB() : 글자의 길이(바이트수)를 나타낸다.
-- 영어는 한 글자당 1BYTE 를 가지게 된다, 그래서 LENGTH 와 LENGTH 값이 같다.
SELECT ENAME, LENGTH(ENAME), LENGTHB(ENAME)
FROM EMP
WHERE DEPTNO = 20;
-- 한글은 한 글자당 3BYTE 를 가지게 된다, 그래서 LENGTH 와 LENGTHB 값이 다르다.
--      ** 한글이 3byte로 고정크기를 가지는 것이 아니다. 버전 또는 다른 DB를 사용하면 2byte기 될 수 있다.
...
--          LPAD(문자열데이터, 자릿수, 특정문자) : 문자열데이터의 지정한 자릿수만큼 왼쪽에 특정문자를 채운다.
SELECT 'AAAAA', LPAD('AAAAA', 10, '*')
FROM DUAL;

-- 9. RPAD() 함수 : 오른쪽에 특정글자를 채운다
--          RPAD(문자열데이터, 자릿수, 특정문자) : 문자열데이터에 지정한 자릿수만큼 오른쪽에 특정문자를 채운다.
SELECT 'AAAAA', RPAD('AAAAA', 10, '*')
FROM DUAL;

-- 휴대폰 번호 또는 주민등록번호 마스킹할 때(가릴 때) 사용가능하다.
SELECT '010-1234-1234' , RPAD(SUBSTR('010-1234-1234',1,9),13,'*') AS RASD_PHONE,
...
--       REPLACE(문자데이터, 문자1, 문자2) : 문자열데이터의 문자1을 문자2로 변경한다.
...
-- 문자 함수 문제
-- 1. EMP 테이블에서 사원 이름의 길이 5 이상인 이름만 출력하라
SELECT ENAME
FROM EMP
WHERE LENGTH(ENAME)>= 5;

-- 2. EMP 테이블에서 JOB 컬럼의 첫 3자리 문자를 *로 대체하여 출력하라
SELECT REPLACE(JOB,SUBSTR(JOB,1,3),'***')
FROM EMP;

-- 3. EMP 테이블에서 ENAME의 마지막 5글자를 추출하고 그 문자열 안에서 A를 @로 변경하여라
--              ENAME 이 5글자 이상인 데이터만 가지고 진행
SELECT REPLACE(SUBSTR(ENAME,-5),'A','@')
FROM EMP
WHERE LENGTH(ENAME) >=5;
...
FROM DUAL;
-- 5. MOD() 함수 : 지정한 DATA 와 숫자를 나눈 나머지를 출력하는 함수
--     MOD(숫자데이터 , 나눌 숫자)
...
-- ROWNUM - 쿼리문을 호출할 때 데이터베이스에서 미리 작성해놓은 순서 번호
--      ** ORDER BY 를 사용하면 쿼리문을 호출한 후에 정렬하기 때문에 ROWNUM이 뒤죽박죽이 될 수 있다.
SELECT ROWNUM, ENAME
FROM EMP
WHERE DEPTNO = 10;
-- EMP 테이블의 14명의 사람을 ROWNUM 나누기 3으로 3명씩 팀 번호를 맞춰주기
SELECT ROWNUM, CEIL(ROWNUM/3)
AS TEAMNO, ENAME
FROM EMP;

-- 날짜 함수
-- DATE 타입의 출력 포맷을 변경하는 방법 1 (코드 작성)
ALTER SESSION SET nls_date_format = 'YYYY-MM-DD HH24:MI:SS';
-- YYYY ( 4자리 년도 ) RR ( 2자리 년도 ) MM (월) DD (일)
-- HH24 ( 24시간형식 ) HH12 ( 12시간 형식 ) MI (분) SS(초)
-- DATE 타입의 출력 포맷을 변경하는 방법 2 (환경 설정)
-- 상단의 도구 - 환경설정 - 데이터베이스 - NLS - 날짜형식 을 변경하면 된다.
...
FROM DUAL;
-- EMP 테이블에서 고용일(HIREDATE) 10주년 되는 날을 사원이름과 고용일, 고용10주년으로 출력하여라
...
FROM DUAL;
-- EMP 테이블에서 사원들이 현재까지 다닌 개월
SELECT ENAME, HIREDATE, SYSDATE, MONTHS_BETWEEN(SYSDATE, HIREDATE) AS "다닌 개월 수"
FROM EMP;
...
--      MM, MONTH, MON : 해당 달을 기준으로 사용 ( 16일 기준으로 월을 올림, 내림 )
...
--      HH, HH12, HH24 : 해당일의 시간을 기준으로 분을 기준으로 일을 올림, 내림
--      MI : 해당일 시간의 초을 기준으로 분을 올림, 내림
SELECT
    sysdate,
...
SELECT TO CHAR(SYSDATE, 'YYYY/MM/DD-HH24:MI:SS') AS 현재날짜시간
FROM DUAL;
SELECT TO_CHAR(SYSDATE, 'YYYY-MM-DD') AS "현재시간",
TO_CHAR(SYSDATE, 'CC') AS CC, -- 세기
TO_CHAR(SYSDATE, 'YYYY') AS YYYY, -- 4자리의 년도
TO_CHAR(SYSDATE, 'YY') AS YY, -- 4자리중 뒤 두자리의 년도 ( 2025년 -> 25를 출력 )
TO_CHAR(SYSDATE, 'MM') AS MM, -- 두 자리수의 월
TO_CHAR(SYSDATE, 'MON') AS MON, -- 월의 약자 ( 1월, JAN )
TO_CHAR(SYSDATE, 'MONTH') AS MONTH, -- 월의 약자 ( 1월, JANUARY)
TO_CHAR(SYSDATE, 'DD') AS DD, -- 오늘의 일(DAY)자
...
-- 날짜를 다른 언어로 출력하는 방법
-- TO_CHAR(날짜 데이터, 날짜출력포맷, 'NLS_DATE_LANGUAGE=ENGLISH')
...
TO_CHAR(SYSDATE, 'HH12:MI:SS PM') AS HH12MISS,
...
TO_CHAR(SAL, 'L999,999') AS SAL_L, -- L 은 현재 서버 나라의 지역(LOCALE) 단위 화폐
TO_CHAR(SAL, '999,999.00') AS SAL_1, -- 0을 적으면 그 자리수 까지 반드시 채운다.
...
-- 자료형 변환 함수를 배웠으니, 쿼리문 작성할 대 같은 자료형끼리 연산을 해주는게 좋다.
SELECT *
FROM EMP
...
FROM EMP;
-- EMP 테이블에서 사원이름과 급여, 수당, 급여+수당을 출력하여라
...
FROM EMP;
-- EMP 테이블에서 사원이름과 연봉을 출력하라 단, 연봉은 SAL * 12 이며 추가 수당이 없으면 연봉에 추가수당을 더한다.
...
-- EMP 테이블에서 사원명, 사원이름, 직책, 급여, 직책에 따른 인상급여를 출력할 것이다.
--          이 때 직책이 MANAGER 면 10%, SALESMAN이면 5%, ANALYST면 동결 그 외에는 3%를 인상할 것이다.
...
-- 1번 문제 EMP 테이블에서 사원번호(EMPNO)와 사원이름(ENAME)을 출력한다.
...
--          단, 하루 급여는 소수점 세번째자리에서 버리고, 시급은 소수점 두번째자리 에서 반올림하라.
...
-- 3번 문제 EMP 테이블에서 사원들은 입사일(HIREDATE) 기준 3개월 지난 후 첫 월요일에 정지원이 된다.
--              사원번호와 사원이름, 입사일, 정직원이 되는날(R_JOB), 추가수당(COMM)을 출력하여라.
--              단, COMM이 없는 사원의 수당은 N/A로 출력하여라.
...
-- 4번 문제 EMP 테이블의 모든 사원을 대상으로 직속 상관의 사원번호(MGR)를 아래와 같은 조건으로 변환해서 출력
--        MGR이 존재하지 않을 때 : 0000
