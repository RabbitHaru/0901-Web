...
select regexp_replace('aaa bbb', '( ){2,}', '')as "one",
       ...
       regexp_replace('aaa   bbb', '( ){2,}', '')as "three" 
from dual;
-- 예제 4. 검색을 할 때 공백을 먼저 입력하고 할 경우에 공백을 제거하고 찾기
...
-- jumin 컬럼을 13자리를 6자리 - 7자리 로 변경해서 출력
-- tel 컬럼을 지역번호, 전화번호 나눠서 출력
select name, jumin, tel,
                    regexp_substr(name, '[^ ]+', 1, 1) as "first name",
                    regexp_substr(name, '[^ ]+', 1, 2) as "last name",
                    regexp_replace(jumin, '([0-9]{6})([0-9]{7})', '\1-\2') as "jumin",
                    regexp_substr(tel, '[^)]+', 1, 1) as "local",
                    regexp_substr(tel, '[^)]+', 1, 2) as "number"
from student;

-- student 테이블에서 주민번호의 앞 6글자를 사용해서
-- 19xx-xx-xx의 형태로 나타내기
-- 주민번호의 7번째 번호를 이용해서 1이면 남자 2면 여자 성별 출력하기.
select name, jumin,
...
-- 조건을 추가해서 함수 사용하기
--      부서 번호가 10번인 사람들 중에서 급여가 최소 급여, 최대 급여 나타내기
...
-- 문제 4. 각 부서-직급별 사원 이름의 두번째 자리에 A 또는 L이 들어가는
--  사원의 수를 출력하세요.
select deptno, job, count(ename)
from emp
where ename like '_A%' -- 첫글자 상관 없고 두번째글자 A 이후 글자 상관 없음
or ename like '_L%' -- 첫갈자 상관 없고 두번째글자 L 이후 글자 상관 없음
group by deptno, job;
...
-- 각 지역 변호 별로 몇명 있는지 출력하라
-- 051 이 몇 명인지 출력ㅎ라(having)
select regexp_substr(tel, '[^)]+',1,1) as "지역번호",
                count(regexp_substr(tel, '[^)]+',1,1)) as "지역번호 사용자 수"
from student
