...
--              ***주의할점 : 출력되는 값이 4000BYTE가 넘어가면 오류가 발생
--          첫번째 인수 : 연결시켜 나타낼 컬럼명
--          두번째 인수 : 컬럼의 데이터를 연결시켜 나타낼 때 사용하는 구분자
-- 예제 ) EMP 테이블에서 부서별 인원의 이름을 나타내어라
...
ORDER BY DEPTNO;
-- ROW가 JOB이 되고 COLUMN이 DEPTNO 가 되게끔 변경
SELECT * FROM ( SELECT DEPTNO, JOB, SAL FROM EMP )
PIVOT ( MAX(SAL)
...
--  예제 ) EMP 테이블에서 SMITH 라는 이름이 이름순으로 했을 때 몇 번째 인지 나타내시오.
...
SELECT P_DATE, P_CODE, P_QTY, P_TOTAL,
        SUM(P_TOTAL) OVER(ORDER BY P_DATE, P_CODE) "TOTAL"
        FROM PANMAE
        WHERE P_STORE = 1000;
...
--                    부서 인원을 나타내어라
SELECT DEPTNO, TRUNC(AVG(SAL)) AS "급여의 평균",
                MAX(SAL) AS "급여의 최대",
                MIN(SAL) AS "급여의 최소",
                COUNT(*)
FROM EMP
...
-- 문제 3. 사원들의 입사연도를 기준으로 부서별로 몇 명이 입사했는지 출력하시오
SELECT TO_CHAR(HIREDATE, 'YYYY') AS "입사연도", DEPTNO, COUNT(*)
FROM EMP
GROUP BY TO_CHAR(HIREDATE, 'YYYY') ,DEPTNO
ORDER BY 입사연도;

-- 문제 4. 추가수당이 있는 사람과 없는 사람의 수를 출력.
...
-- 책P212 문제 5번. EMP테이블을 사용하여 직원들의 급여와 급여의 전체 급여의
...
FROM EMP;
-- 책P212. 6번문제 , FRULT 테이블을 참고해서 아래와 같이 만드세요.
...
SELECT REGEXP_SUBSTR(TEL,'[^)]+',1,1) AS "LOCAL" FROM STUDENT;
SELECT * FROM (SELECT REGEXP_SUBSTR(TEL,'[^)]+',1,1) AS "LOCAL" FROM STUDENT)
PIVOT ( COUNT(LOCAL)
        FOR LOCAL IN ('02' AS "SEOUL", '031' AS "GYEONGGI", '051' AS "BUSAN",
                    '052' AS "ULSAN", '053' AS "DAEGU", '055' AS "GYEONGNAM")
                    );
...
ORDER BY empno;
--		select 구문에 모든 컬럼에 작성해주는 거를 추천한다.
