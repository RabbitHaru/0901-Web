...
-- tip) 보통 모든 select문에 * 작성하지 않고 모든 컬럼명을 나열하는게 좋다.
--		-> 쿼리 결과문이 항상 똑같은 순서로 나오지는 않을 수도 있기 때문

-- JOIN 의 종류
-- 1. 등가 조인 : 조인의 조건이 (=) 조건으로 붙이는 조인
-- 2. 비등가 조인 : 조인의 조건이 ( >=, >, <=, < 등 ) 같다가 아닌 조건으로 붙
-- 3. 셀프 조인 : 하나의 테이블을 조인
--		등가, 비등가, 셀프 조인은 inner join 이라고 한다.
--		innver join : 데이터가 잇어야 출력하는 조인으로 붙이는 조인
-- 4. 외부 조인(outer join) : 데이터가 없어도 출력하는 조인

-- 등가 조인
-- 예제 ) 각 사원들의 사원정보, 사원명, 부서번호, 부서명을 출력하여라.
...
-- 예제 ) 학생들 중 학과1의 번호가 101번인 학생들의 이름과 지도교수 이름을 출력
...
--		지도교수가 없는 학새오가 지도학생이 없는 교수 모두 출력하여라.
...
-- 예제 ) 부서에 대한 정보를 모두 보여주고, 부서번호가 '20'인 사원의 번호, 이름, 급여를 출력
--		기호를 작성하지 않았을 때
...
-- 구분 했을때 
SELECT E.EMPNO , e.ENAME , E.JOB , D.DEPTNO , D.DEPTNO , D.LOC 
...
-- 1번문제 ) 급여(SAL)가 2000 초과인 사원들의 부서 정보, 사원정보를 출력하라
...
SELECT d.DEPTNO , d.DNAME , avg(e.SAL) AS "AVG_SAL",
		min(e.SAL) AS "MIN_SAL", MAX(e.sal) AS  "MAX_SAL", COUNT(D.DNAME)
FROM emp e, DEPT d
WHERE 
...
-- 5번 문제 ) 현재 직급이 있는 사원의 이름과 직급, 현재 연봉,
--			해당직급의 하한 금액과 상한 금액을 출력하라.
--			사원정보(emp2) : 이름(name), 직급(position), 연봉(pay)
--			연봉 정보(p_grade) : 직책(position) 하한금액(s_pay) 상한금액(e_pay)
...
-- 6번 문제 ) 사원의 이름과 나이, 현재 직급, 예상직급을 출력
--			나이는 birthday를 사용해서 현재 날짜와 계산 후 -12 를 한다.
--			사원정보(emp2)
--			직급정보(p_grade) : s_age, e_age를 이용해서 예상 직급을 나타낼수 있다.
select * FROM p_grade;
...
--			Notebook 을 선택 할 수 있는 고객의 이름과 포인트, 상품명을 출력하라.
...
-- 8번 문제 ) 교수 테이블에서 교수의 번호, 이름, 입사일, 자신보다 입사일이 빠른 사람 수를 출력
...
--			단, 부서정보, 사원번호 순으로 출력
--			부서정보 (dept) : 부서번호, 부서이름
-- 			사원정보 (emp) : 사원번호, 사원이름, 급여
--			급여등급정보 ( salgrade ) : 급여등급
--			직속상관의 정보 ( emp ) : 사원정보의 mgr에 따른 직속상관 이름
SELECT * FROM salgrade;

-- ORACLE JOIN 문법
SELECT d.DEPTNO , d.DNAME ,e1.EMPNO ,e1.ENAME ,e1.SAL ,
		s.GRADE , e1.mgr, e2.ENAME 
