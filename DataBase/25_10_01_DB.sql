...
--			숫자로 시작할 수 없고, 포함은 가능하다.
--			특수기호도 사용가능하지만 _를 제외한 것은 추천하지 않는다.
--		2. 테이블 이름은 최대 30byte 까지 사용 가능
--			한글로 테이블을 생성시에 주의해야함
--		3. 테이블 이름은 같은 사용자일 때 중복되어서는 안된다.
--			scott 라는 꼐정에 emp 테이블이 2개가 될 순 없다.
...
-- 임시 테이블 생성하기
--		임시 테이블이란 ? 데이터를 생성하지만, 일시적으로 테스트할 테스트를 말한다.
--				COMMIT 이나 세션이 종료될 경우에 삭제된다.
--			COMMIT ? 트랜잰션을 성공적적으로 종료시킨다 라는 뜻의 명령어
--			트렌잭션 : 명령을 하는 기간
...
-- 테이블 자체를 삭제했기 때문에 확인조차 불가능하다. ( 에러 뜨는게 정상 )
SELECT * FROM dept_test4;
...
--	오라클 데이터베이스는 데이터베이스 운영에 관한 데이터를 데이블로 가지고있다.
--	큰 틀로 나누게되면
...
--				DBA_XXXX  : DB 관리를 위한 정보(sys, system만 열람)
...
-- scott 계정에 대한 정보 확인 (sys, system 계정에서 확인가능 )
...
--			추가하고, 기본적으로 오늘 날짜가 자동으로 입력되기끔 하세요.
...
-- CRUD ( CTEATE READ UPDATE DELETE )
...
--			values(값)
INSERT INTO prof_test(profno, name, id, POSITION, pay, hiredate)
VALUES (1002,'hello','helo','hello prof', 300, '2024-10-01');
...
-- 테이블이 가진 컬럼에 대한 데이터를 values 모두 작성해주세면,
...
-- 예제 ) 교수 번호가 1000번 에서 1999번까지인
--		교수의 번호와 교수 이름은 prof_test2 에 저장하고,
...
ALTER TABLE prof_test3 DROP COLUMN id;
ALTER TABLE prof_test3 DROP COLUMN POSITION;
ALTER TABLE prof_test3 DROP COLUMN pay;
ALTER TABLE prof_test3 DROP COLUMN hiredate;
ALTER TABLE prof_test3 DROP COLUMN bonus;
ALTER TABLE prof_test3 DROP COLUMN deptno;
ALTER TABLE prof_test3 DROP COLUMN email;
ALTER TABLE prof_test3 DROP COLUMN hpage;
...
-- 예제 2) prof_test 테이블에서 'James Bond'라는 이름을 가진 교수의
--			pay를 350으로 변경하라.
UPDATE prof_test
SET pay = 350
...
--		트랜잭션 과니하는 언어 TCL (Transaction Control Language)
