...
--			1. WHERE절 연산자 오른쪽에 위치해야하며, 반드시 소괄호로 묶어야한다.
...
--			서브 쿼리의 수행 결과 값 1개를 메인 쿼리로 전달해서 메인쿼리를 수행하게 된다.
...
--		예제 1 ) STUDENT 테이블과 DEPARTMENT 테이블을 사용하여 'Anthony Hopkins'
...
SELECT p.NAME AS prof_name, p.HIREDATE AS HIREDATE, d.DNAME AS prof_name
...
FROM STUDENT s 
WHERE s.WEIGHT > ( SELECT avg(s.WEIGHT)
					FROM STUDENT s 
					WHERE s.DEPTNO1 = 201);

--			2. 다중행 sub query : sub query의 수행 결과 값이 2개의 행 이상 출력되는 것
...
--					ANY와 ALL의 예시. ( 서브쿼리의 결과 값이 100, 200, 300 이라고 가정 )
...
--			1. 근무지역이 'Pohang Main Office'인 부서 번호를 조회학
SELECT dcode
FROM DEPT2
WHERE area = 'Pohang Main Office';
--			2. 얻은 부서 번호를 이용해서 사원번호를 출력하기 (Main query 작성)
...
--				2. 다중행 연산자 (> ANY ) 를 이용하여 메인쿼리 작성하기
...
--			문제 2 ) Student 테이블을 조회하여 전체 학생 중에서 체중이 2학년 학생들의 체중에서
...
FROM STUDENT s 
WHERE s.WEIGHT < all ( SELECT s.WEIGHT 
						FROM STUDENT s 
						WHERE s.GRADE = 2);
--				3. 단일행 연산자를 이용하여 메인쿼리 작성하기
SELECT s.NAME , s.GRADE , s.WEIGHT 
FROM STUDENT s 
WHERE s.WEIGHT < ( SELECT min(s.WEIGHT) 
						FROM STUDENT s 
						WHERE s.GRADE = 2);
...
--	`				부서명, 직원명, 연봉을 출력하세요.
--				1. 각 부서별 평균 연봉을 구하기
SELECT avg(e.pay), e.DEPTNO  --서브 쿼리 사용할 때는 확인용인 deptno를 빼고 작성한다.
FROM EMP2 e 
GROUP BY e.DEPTNO;
--				2. 다중행 연산자를 이용해서 메인쿼리 작성하기
SELECT d.DNAME , e.NAME , e.PAY 
FROM EMP2 e , DEPT2 d
WHERE e.DEPTNO = d.DCODE 
and pay < ALL ( SELECT avg(e.pay)
					FROM EMP2 e 
					GROUP BY e.DEPTNO);
...
FROM PROFESSOR p, DEPARTMENT d 
WHERE p.deptNO = d.DEPTNO
AND (p.DEPTNO,p.HIREDATE) IN ( SELECT p.DEPTNO , min(p.HIREDATE) 
								FROM PROFESSOR p 
								GROUP BY p.DEPTNO)
ORDER BY p.HIREDATE;
...
--			1. 해당 직급별 연봉을 구하기
SELECT e.POSITION, max(e.PAY)
FROM EMP2 e
group BY POSITION;
--			2. 해당 직급에서 최대 연봉을 받는 직원의 이름, 직급, 연봉을 출력하기.
SELECT NAME ,"POSITION" ,PAY
FROM EMP2 
WHERE ("POSITION",PAY ) IN ( SELECT e.POSITION, max(e.PAY)
								FROM EMP2 e
								group BY POSITION)
ORDER BY PAY;
...
--			-> 그 값을 입력(MAIN의 조건 )과 출력(SCALAR의 결과)로 메모리에 저장
--		4. 다음 조건이 들어오면 메모리 확인 후 있으면 메모리에서 출력이 없으면 위 과정을 반복

--	** 작동원리를 식당으로 예를 들어보면,
--		식당의 메뉴가 1개만 있다면 많은 손님이 와도 메뉴를 미리 만들어 놓을 수 있다.
--		식당의 네뮤가 100개가 있다면 많은 손님이 오면 메뉴를 만들어 내는 것이 느릴 거다.
...
--		 	따라서, 중복된 데이터가 있을 때는 스카라 서브쿼리를 사용할 수 없다.
...
--			    끝 번호 : 110
--				증가 값 : 1
--				반복될 것이며 캐싱은 2개씩 ( 다시 시작할 땐 90 부터 시작 )
...
--				>> 주의 : 어디든 작성하기만ㅎ면 시퀀스 값이 증가 또는 감소한다.
--			시퀀스.currval : nextval 출력한 마지막 번호를 알려준다.
SELECT jno_seq.currval FROM dual;

--		4. 시퀀스를 이용하여 maxvalue / minvalue / cycle 확인하기
BEGIN
	FOR i IN 1..9 LOOP
...
MAXVALUE 20
START WITH 10;
--		cycle cache를 지정하지 않았다.
--			-> cycle은 nocycle이 기본값, cache는 cache 20이 기본값(오라클 기준)
CREATE TABLE s_rev1(NO number);

INSERT INTO s_rev1 VALUES (jno_seq_rev.nextval);
-- 6번까지는 수행하며, 7번째에는 에러를 출력한다.
--			> 10에서 -2 씩 10,8,6,4,2,0 까지 뽑고 cycle이 아니기 때문에
...
--			수정 : ALTER SEQUENCE 시퀀스명 아레에 수정할 조건을 작성하면 된다.
--			예제 ) 기존의 jno_seq 의 최대값을 120으로 변경하고, cache 값을 10으로 수정
...
--				  CREATE SEQUENCE 시퀀스명
--				2. INCREMENT BY 조정
--					예제 ) 현재 시퀀스가 105인데 1부터 다시 시작하고 싶을때
--					INVREMENT BY -104 수정 후
--					시퀀스.NEXTVAL ---> 시퀀스 값이 1로 바뀐다.
--					이후에 INVREMENT BY 1로 재수정
...
--				단, 급여가 많은 순으로 정렬하되, 같으면 사원번호를 기준으로 오름차순 정렬할 것 
--				1. 전체 사원의 평균 급여 구하기
SELECT avg(sal)
FROM EMP;
--				2. emp, dept, salgrade 를 join 하여 메인 쿼리 작성
...
--		사원번호, 사원이름, 직책, 부서번호, 부서명, 근무 위치를 출력하세요.
--		1. 30번 부서에 존재하는 직책을 구하기
SELECT job -- distinct job (중복 제거 가능)-group by 안써도됨
FROM emp
WHERE DEPTNO = 30
GROUP BY job;
