...
--	서버에 문자열 출력하기
SET SERVEROUTPUT ON;
--		서버에 문자영을 출력하기 위해 환경변수를 변경하는 명령어
...
--	선언부 ( DECLARE )
--		변수 및 상수를 선언, 값을 초기화 하는 부분
--		변수 : 설정한 값을 계속 변경이 가능
--		변수를 선언 > 변수명 자료형(크기) := 값(DATA)
...
--			IF 조건1 THEN 실행문		1번 조건에 의한 실행 (선택)
...
--				ELSE 실행문
--			END CASE;
--			예제 ) 학생 점수를 입력받아 점수 / 10을 안 후 나온 점수가 10과 9면 A
...
END;
--			** 반복문을 빠져나가는 방법
--			1. EXIT WHEN [조건식] 을 통해서 조건이 맞으면 EXIT명령 실행
--			2. IF 문을 사용해서 EXIT 명령을 실행
...
--				반복 수행 작업;
--			END LOOP;
--			예제 ) WHILE LOOP을 사용해서 0부터 4까지 출력하여라.
DECLARE
	V_NUM NUMBER := 0;
BEGIN
...
--				2. CONTINUE WHEN [조건식]을 사용하여
--					조건식이 맞으면 CONTINUE 명령어를 실행시키기
--			예제 ) FOR LOOP를 사용해서 0부터 10까지 홀수만 출력하기
BEGIN
...
BEGIN
	DBMS_OUTPUT.PUT_LINE(' FOR LOOP');
	FOR i IN 1..30 LOOP
		IF MOD(i,5 ) = 0 THEN DBMS_OUTPUT.PUT_LINE('5의 배수 : ' || i);
		END IF;
	END LOOP;
END;
...
--		예제 ) 레코드를 이용하여 테이블에 데이터 INSERT, UPDATE\
...
DECLARE
	TYPE COL_EX IS TABLE OF VARCHAR2(20)
	INDEX BY VARCHAR2(10); -- 문자형 인덕세를 할 때는 크기를 작동해야한다.
	TEXT_ARR COL_EX;
BEGIN
	TEXT_ARR('A') := 'AAA';
   	TEXT_ARR('B') := 'BBB';
...
BEGIN
	...
	-- ()안의 쿼리문의 결과 ROW가 하나씩 카운터변수 i에 대입된다.
		idx := idx+1;
...
--			COUNT	> 컬렉션에 몇 개의 INDEX가 저장되어 있는지
...
BEGIN
	...
	TEXT_ARR(7) := '7TH DATA';
	IF TEXT_ARR.EXISTS(3) THEN DBMS_OUTPUT.PUT_LINE('있다!!');
...
--	문제 1 ) EMP 테이블을 복사해서 연습용테이블 EMP_RECORD 명으로 데이터 없이 구조만 생성
...
--		3번예외 [또는 4번예외]가 발생하면 실행문2을 실행한다.
...
--		** 기본적인 쿼리문, DDL, DML 등에서는 사용 불가능하다.
DECLARE
	V_WRONG NUMBER;
BEGIN
	SELECT DNAME INTO V_WRONG
...
--			부서번호를 입력받게 한다. 입력받은 번호에 다라서 쿼리문을 실행하고
--			쿼리문을 실행한 값 중 DNAME을 출력하여라.
--			10을 입력하면 -> ACCOUNTING
