...
console.log(0.1 + 0.2); // 자바스크립트는 실수 계산을 정확하게 하지 못한다.
...
// 문자열 * 숫자 -> 문자열을 숫자로 변경해준다.
// 문자열 + 숫자 -> 숫자를 문자열로 변경해준다.
let text = '50';
let num = 5;
...
// 논리 연산자 :  여러 가지 조건을 가팅 확인할 때 사용하는 연산자
//        OR 연산자 ( || ) , AND 연산자 ( && ) , NOT 연산자 ( ! )
console.log(1 > 2 || 1 < 2);
console.log(1 > 2 && 1 < 2);
console.log(!(1 > 2));

// 조건문 if-else : 조건이 true 라면 실행문을 실행 false라면 else문을 실행한다.
...
switch (session) {
    case "1": console.log("마케팅 세션은 201호에서 진행합니다.");
        break;
    case "2": console.log("개발 세션은 203호 에서 진행합니다.");
        break;
    case "3": console.log("디자인 세션은 205호 에서 진행합니다.");
        break;
...
// 1~ 100까지의 숫자들 중에서 3의 배수만 document.write로 출력하여라.
...
// 반복문 ( while ) - 조건이 false가 될 때까지 실행문을 반복한다.
//         > while(조건){ 실행문 }
...
// 반복문 ( do ~ while ) - 무조건 한 번 실행하고, 조건을 확인하며 참이면 실행문을 반복한다.
// let stars = parseInt(prompt("몇 개의 별을 표시할까요?"));
// do {
//     document.write('*');
//     stars--;
...
// 문제 1. 월드컵은 4년에 한 번 개최된다. 2002년 부터 2050년까지의 월드컵이 개최되는 년도를 출력.
//          출력은 document.write 로 출력
// let year = 2002;
// while(year<=2050){
