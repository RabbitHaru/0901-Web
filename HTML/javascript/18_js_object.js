...
// 인스턴스란 ? 객체를 복사해서 변수에 저장하는 것 ( new 키워드를 사용한다.)
// 프로퍼티란 ? 객체의 특징이나 속성
// 메서드란 ? 객체에서 할 수 있는 동작(함수)

// 인스턴스는 객체의 프로퍼티와 메서드를 그대로 물려받는다.
// 프로퍼티와 메서드를 사용할 때는 인스턴스명.프로퍼티명 또는 메서드명을 작성한다.
...
let charsNums = chars.concat(nums); // chars와 nums를 순서대로 합친다.
...
//          pop(), shift() 메서드 : 기존 배열에 맨 끝(앞)에 요소를 꺼내는 메서드
let pop1 = chars.pop();
...
//                              삭제하고 배열 반환한 다음, 그 자리에 요소(세번째인수)를 추가한다.
...
//           인수가 1개일 때 : 지정한 인덱스(첫번째인수) 부터 마지막 요소까지 추출한다.
let slice1 = colors.slice(2);
console.log(`slice() 메서드 : ${slice1}`);
console.log(`원래의 배열 : ${colors}`);
//           인수가 2개일 때 : 지정한 인덱스(첫번째인수)부터 지정한 인덱스(두번째인수) 직전의 인덱스까지 추출한다.
...
let date2 = new Date("2024-02-25T12:00:00"); // 특정 날짜를 더해서 시간까지 지정할 수 있다.

// Date의 메서드들
// Date의 날짜/시간 정보를 가져오는 메서드들
console.log(today.getFullYear()); // 4자리 년도
console.log(today.getMonth()); // 0 ~ 11까지 월 ( 사용자에게 보여줄 때는 +1 을 해줘야한다. )
...
// setDat1 의 날짜/시간을 2024년 1월 1일 0시 10분 20.500초로 지정
...
let firstDay = new Date("2020-12-27"); // 시작 날짜를 인스턴스로 지정
...
(function() {
    // window.open() 메서드는 팝업차단 등의 이유로 창을 띄우지 못할 시에 null 을 반환한다.
     let newIn = window.open('./notice.html', 'pop', 'width= 500, height=400, left=100, top=100');
    if( newIn == null ){
