...
export default function ArticleList() {
  ...
  const pageClick = async(e) => {
    const num = e.target.getAttribute("data-num");
      try {
        const response = await auth.get(`/articleList?page=${num}&${articleList.pageRequestDTO?.link}`);
        setArticleList(response.data);
      } catch (e) {
        console.log(e);
      }
  ...
  const searchClick = async () => {
      try {
        const response = await auth.get(`/articleList?page=1&size=10&types=${typeRef.current.value}&keyword=${keywordRef.current.value}`);
        setArticleList(response.data);
      } catch (e) {
        console.log(e);
      }
        ...
        <ul className="flex-wrap pagination">
          {articleList?.prev && (
            <li className="page-item">
              <a className="page-link" data-num={articleList.start - 1} onClick={pageClick}>
                이전
              </a>
            </li>
          )}
          ...
          {articleList?.next && (
            <li className="page-item">
              <a className="page-link" data-num={articleList.end + 1} onClick={pageClick}>
                다음
              </a>
            </li>
          )}
