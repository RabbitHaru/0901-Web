...
public class BlogViewController {
    ...
    public String findArticle(@PathVariable("id") long id, PageRequestDTO pageRequestDTO, Model model){
        Article article = blogService.findById(id);
        model.addAttribute("article", new ArticleViewResponse(article));
        return "article";
    }
    @GetMapping("/new-article")
    public String newArticle(@RequestParam(required = false) Long id, PageRequestDTO pageRequestDTO, Model model){
        if(id == null){
            // 글 추가
